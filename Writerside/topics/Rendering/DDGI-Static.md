...
Static DDGI uses the same probe volumes and the same spherical harmonic representation as
[Dynamic DDGI](DDGI-Dynamic.md), but resolves irradiance once at bake time instead of continuously at
runtime. It is the low-end and no-ray-tracing path.
...
**Better bounce fidelity than traditional baked lighting.** The spherical harmonic representation captures
directional irradiance rather than a flat lightmap value, so surfaces respond correctly to their orientation
...
**No DXR requirement at runtime.** Once baked, there is no ray tracing. This is what makes Static DDGI
viable on ultra-low-end GPUs and on hardware with no ray tracing support at all.

## When to use it

| Situation | Recommendation |
|---|---|
| Minimum spec has no DXR support | Static DDGI |
| Minimum spec is very low-end but DXR capable | Static DDGI as a scalability level |
| Lighting is fully static (no time of day, no destructible lights) | Static DDGI is sufficient |
| Lighting changes at runtime | [Dynamic DDGI](DDGI-Dynamic.md) |
| Shipping across a wide hardware range | Both, switched by scalability |
